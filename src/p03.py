...
s_array = np.array(list(map(lambda x: list(x), s_list)))
n_array = np.array(list(map(lambda x: [1 if not ch.islower() else 0 for ch in x], s_list)))

for i in range(3, 1247):
    for j in range(3, 77):
        if n_array[i][j] == 0:
            if sum(n_array[i, j-3:j+4]) == 6:
                if j - 4 != -1 and n_array[i, j - 4] == 1:
                    continue
                if j + 4 != 80 and n_array[i, j + 4] == 1:
                    continue
...
import re
print("".join(re.findall("[^A-Z]+[A-Z]{3}([a-z])[A-Z]{3}[^A-Z]+", raw_string)))
